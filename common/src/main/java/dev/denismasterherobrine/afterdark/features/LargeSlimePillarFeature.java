...
public class LargeSlimePillarFeature extends Feature<LargeDripstoneFeatureConfig> {
    public LargeSlimePillarFeature(Codec<LargeDripstoneFeatureConfig> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneHelperMixin.invokeCanGenerateOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof CaveSurface.Bounded) {
                ...
                if (column$range.getHeight() < 4) {
                    ...
                    LargePillar largepillarfeature$largepillar = makeSlime(blockpos.withY(column$range.getCeiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargePillar largepillarfeature$largepillar1 = makeSlime(blockpos.withY(column$range.getFloor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
    ...
    private static LargePillar makeSlime(BlockPos pRoot, boolean pPointingUp, net.minecraft.util.math.random.Random pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
