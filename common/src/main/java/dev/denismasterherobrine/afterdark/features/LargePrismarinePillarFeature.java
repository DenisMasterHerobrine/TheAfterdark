...
public class LargePrismarinePillarFeature extends Feature<LargeDripstoneFeatureConfig> {
    public LargePrismarinePillarFeature(Codec<LargeDripstoneFeatureConfig> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneHelperMixin.invokeCanGenerateOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof CaveSurface.Bounded) {
                ...
                if (column$range.getHeight() < 4) {
                    ...
                    LargePillar largepillarfeature$largepillar = makePrismarine(blockpos.withY(column$range.getCeiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargePillar largepillarfeature$largepillar1 = makePrismarine(blockpos.withY(column$range.getFloor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
    ...
    private static LargePillar makePrismarine(BlockPos pRoot, boolean pPointingUp, net.minecraft.util.math.random.Random pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
