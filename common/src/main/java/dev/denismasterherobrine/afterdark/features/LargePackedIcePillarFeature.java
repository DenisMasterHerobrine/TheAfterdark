...
public class LargePackedIcePillarFeature extends Feature<LargeDripstoneFeatureConfig> {
    public LargePackedIcePillarFeature(Codec<LargeDripstoneFeatureConfig> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneHelperMixin.invokeCanGenerateOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof CaveSurface.Bounded) {
                ...
                if (column$range.getHeight() < 4) {
                    ...
                    LargePillar largepillarfeature$largepillar = makeIce(blockpos.withY(column$range.getCeiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargePillar largepillarfeature$largepillar1 = makeIce(blockpos.withY(column$range.getFloor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
    ...
    private static LargePillar makeIce(BlockPos pRoot, boolean pPointingUp, net.minecraft.util.math.random.Random pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
    ...
    static final class LargePillar {
        ...
        void placeBlocks(StructureWorldAccess pLevel, net.minecraft.util.math.random.Random pRandom, WindOffsetter pWindOffsetter, int pRadius) {
                    ...
                    if (!(f > (float)this.radius)) {
                        ...
                        if (k > 0) {
                            ...
                            for(int i1 = 0; i1 < k && blockpos$mutableblockpos.getY() < l; ++i1) {
                                ...
                                } else if (flag && pLevel.getBlockState(blockpos).isIn(BlockTags.BASE_STONE_OVERWORLD) || flag && pLevel.getBlockState(blockpos).isOf(Blocks.ICE) || flag && pLevel.getBlockState(blockpos).isOf(Blocks.PACKED_ICE)) {
                                    break;
                                }
