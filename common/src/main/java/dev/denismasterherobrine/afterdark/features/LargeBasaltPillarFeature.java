...
public class LargeBasaltPillarFeature extends Feature<LargeDripstoneFeatureConfig> {
    public LargeBasaltPillarFeature(Codec<LargeDripstoneFeatureConfig> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneHelperMixin.invokeCanGenerateOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof CaveSurface.Bounded) {
                ...
                if (column$range.getHeight() < 4) {
                    ...
                    LargePillar largepillarfeature$largepillar = makeBasalt(blockpos.withY(column$range.getCeiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargePillar largepillarfeature$largepillar1 = makeBasalt(blockpos.withY(column$range.getFloor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
    ...
    private static LargePillar makeBasalt(BlockPos pRoot, boolean pPointingUp, Random pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
    ...
    static final class LargePillar {
        ...
        void placeBlocks(StructureWorldAccess pLevel, Random pRandom, WindOffsetter pWindOffsetter) {
    ...
    static final class WindOffsetter {
        ...
        BlockPos offset(BlockPos pPos) {
            if (this.windSpeed == null) {
                ...
                Vec3d vec3 = this.windSpeed.multiply((double)i);
